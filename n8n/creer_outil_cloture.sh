...
: "${N8N_API_KEY:?Définis d'abord: export N8N_API_KEY=...}"
...
# ---------------------------------------------------------------------------
SUB_WF_BODY=$(jq -n \
  --arg doc "$SHEETS_DOC_ID" \
  --arg credId "$SHEETS_CRED_ID" \
  --arg credName "$SHEETS_CRED_NAME" \
  '{
    ...
    nodes: [
      ...
      },
      {
        ...
        parameters: {
          ...
          sheetName:  { __rl: true, mode: "name", value: "={{ $json.onglet }}" },
          options: {}
        },
...
# Active le sous-workflow (Execute Workflow Trigger) — best effort
...
echo "$CUR" | jq -e '.nodes' >/dev/null || { echo "ERREUR: impossible de lire le serveur MCP."; echo "$CUR"; exit 1; }

# Garde-fou : ne rien faire si un node "Lire_Classeur_Cloture" existe déjà
if echo "$CUR" | jq -e '.nodes[] | select(.name=="Lire_Classeur_Cloture")' >/dev/null; then
  ...
  NODE=$(jq -n --arg sub "$SUB_ID" '{
    ...
    position: [280,430],
    parameters: {
      description: "Lit un onglet du classeur de clôture 2025. Onglets utiles : 07_Rapprochement_Factures (rapprochements/impayés), 08_Anomalies, 09_Soldes_Bancaires (cohérence des soldes), 10_Statistiques (tableau de bord), 11_Logs_Workflow. Filtre optionnel par colonne/valeur, ex. Statut_Rapprochement = 'Non rapproché'.",
      workflowId: { __rl: true, value: $sub, mode: "list", cachedResultName: "VVL — MCP · Lire Classeur Clôture (sous-WF)" },
      workflowInputs: {
        mappingMode: "defineBelow",
        value: {
          "onglet":         "={{ $fromAI('onglet','Nom exact de l onglet du classeur de clôture 2025 à lire (ex. 07_Rapprochement_Factures, 08_Anomalies, 09_Soldes_Bancaires, 10_Statistiques, 11_Logs_Workflow).','string') }}",
          "filtre_colonne": "={{ $fromAI('filtre_colonne','Nom de la colonne sur laquelle filtrer (optionnel).','string','') }}",
          "filtre_valeur":  "={{ $fromAI('filtre_valeur','Valeur recherchée dans la colonne de filtre (optionnel).','string','') }}"
        },
        matchingColumns: [],
        schema: [
  ...
  # Construit le corps PUT : seulement name/nodes/connections/settings, node + connexion ajoutés
...
echo
echo "✅ Terminé."
echo "   Sous-workflow : ${SUB_ID}"
echo "   Serveur MCP   : ${MCP_WF_ID} (outil Lire_Classeur_Cloture ajouté & actif)"
